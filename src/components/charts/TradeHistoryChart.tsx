...
export const TradeHistoryChart = ({ chartMarkers }: Props) => {
  ...
  return (
    <section className="relative">
      ...
      ></div>
      {isLoading ? (
        <span className="top-1/2 left-1/2 z-10 absolute text-success -translate-x-1/2 -translate-y-1/2 loading loading-infinity loading-lg transform"></span>
      ) : null}
      {!dailyTrades ||
        (dailyTrades.length === 0 && (
          <div>
            <span className="top-1/2 left-1/2 z-10 absolute text-success -translate-x-1/2 -translate-y-1/2 loading loading-dots loading-lg transform"></span>
            <span className="top-1/2 left-1/2 z-10 absolute mt-7 text-success -translate-x-1/2 -translate-y-1/2 transform">
              No Data Available
            </span>
          </div>
        ))}
